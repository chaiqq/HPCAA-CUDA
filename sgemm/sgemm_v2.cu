 ...
 * C: M rows, N cols
 */
//TODO: implement v2. could be only focus on square matrices
#include <cuda.h>
...
template <
    const int TILE_SIZE_M,  // height of block of C that each  block calculate
    const int TILE_SIZE_K,  // width of block of A that each  block load into shared memory
    const int TILE_SIZE_N,  // width of block of C that each  block calculate
...
__global__ void sgemm_2(float * __restrict__ A, float * __restrict__ B, float * __restrict__ C,
    ...
    for(int phase = 0; phase < std::ceil(k / (float) blockDim.x); ++phase){
        float Aelem = 0.0f, Belem = 0.0f;
        shr_A[ty][tx] = 0.0f;
        shr_B[ty][tx] = 0.0f;
        if(row < m && phase * TILE_SIZE + tx < k){
            Aelem = A[row * k + phase * TILE_SIZE + tx];
        }
...
int main(){
    ...
    sgemm_2<block_size> <<<GRID, BLOCK>>> (dev_A, dev_B, dev_C, M, K, N);
...
/*
    ...
    for (int i = 0; i < numRepeats; ++i){
        cublasSgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N, N, K, M, &alpha, dev_B, N, dev_A, K, &beta, dev_C, N);
    }
