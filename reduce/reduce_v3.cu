...
void __global__ reduce_3(float *d_in, float *d_out){
...
void  reduce(float *d_in, float *d_out, float *d_intermediate, dim3 block, dim3 grid){
    reduce_3<<<grid, block, block.x * sizeof(float)>>>( d_in, d_intermediate);
    reduce_3<<<1, grid.x / 2, grid.x* sizeof(float) >>>(d_intermediate, d_out);
}
