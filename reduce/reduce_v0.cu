...
void __global__ reduce_0(float *d_in, float *d_out){
...
void  reduce(float *d_in, float *d_out, float *d_intermediate, dim3 block, dim3 grid){
    reduce_0<<<grid, block, block.x * sizeof(float)>>>( d_in, d_intermediate);
    reduce_0<<<1, grid.x, grid.x* sizeof(float) >>>(d_intermediate, d_out);
}
