...
constexpr unsigned int  GRID_SIZE = N / BLOCK_SIZE / 4;
...
//     if(blockSize >= 32) sdata[tid] += _shfl_down_sync(0xffffffff,sum,16);
...
template <unsigned int blockSize>
void __global__ reduce_5(float *d_in, float *d_out){
...
void  reduce(float *d_in, float *d_out, float *d_intermediate, dim3 block, dim3 grid){
    reduce_5<BLOCK_SIZE> <<<grid, block, block.x * sizeof(float)>>>( d_in, d_intermediate);
    reduce_5<GRID_SIZE> <<<1, grid.x / 2, grid.x* sizeof(float) >>>(d_intermediate, d_out);
}
